...
check("multi-word search spans fields", M.filterEvents(payload.events, "alice Kontoret", null).length, 1)
check("multi-word search is order-independent", M.filterEvents(payload.events, "kontoret standup", null).length, 1)
check("multi-word search needs every word", M.filterEvents(payload.events, "standup nattpass", null).length, 0)
check("search ignores surrounding whitespace", M.filterEvents(payload.events, "  standup  ", null).length, 1)
check("search words match inside a run-on title", M.filterEvents(payload.events, "stand up", null).length, 1)
check("a run-on search matches a spaced title", M.filterEvents(payload.events, "utansluttid", null).length, 1)
check("a run-on search still needs one value to hold it", M.filterEvents(payload.events, "standupkontoret", null).length, 0)
check("a run-on search does not fuse two attendees", M.filterEvents(payload.events, "combob", null).length, 0)
